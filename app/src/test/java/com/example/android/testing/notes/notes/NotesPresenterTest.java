/*
 * Copyright 2015, The Android Open Source Project
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package com.example.android.testing.notes.notes;

import com.example.android.testing.notes.data.Note;
import com.example.android.testing.notes.data.NotesRepository;
import com.example.android.testing.notes.data.NotesRepository.LoadNotesCallback;
import com.google.common.collect.Lists;

import org.junit.Before;
import org.junit.Test;
import org.mockito.ArgumentCaptor;
import org.mockito.Captor;
import org.mockito.InOrder;
import org.mockito.Mock;
import org.mockito.Mockito;
import org.mockito.MockitoAnnotations;

import java.util.ArrayList;
import java.util.List;

import static junit.framework.Assert.fail;
import static org.mockito.Matchers.any;
import static org.mockito.Mockito.times;
import static org.mockito.Mockito.verify;

public class NotesPresenterTest {

    private static List<Note> notesList = Lists.newArrayList(new Note("Title1", "Description1"),
            new Note("Title2", "Description2"));

    private static List<Note> emptyNotesList = new ArrayList<>(0);

    @Mock
    private NotesRepository notesRepository;

    @Mock
    private NotesContract.View notesView;

    @Captor
    private ArgumentCaptor<LoadNotesCallback> mLoadNotesCallbackCaptor;

    private NotesPresenter notesPresenter;

    @Before
    public void setupNotesPresenter() {
        // Mockito has a very convenient way to inject mocks by using the @Mock annotation. To
        // inject the mocks in the test the initMocks method needs to be called.
        MockitoAnnotations.initMocks(this);

        // Get a reference to the class under test
        notesPresenter = new NotesPresenter(notesRepository, notesView);
    }

    @Test
    public void loadNotesFromRepositoryAndLoadIntoView() {
        // Given an initialized NotesPresenter with initialized notes
        // When loading of Notes is requested
        notesPresenter.loadNotes(true);

        // Callback is captured and invoked with stubbed notes
        // Capture is going to capture the callback
        verify(notesRepository).getNotes(mLoadNotesCallbackCaptor.capture());
        // Then notesList simulates a real response
        mLoadNotesCallbackCaptor.getValue().onNotesLoaded(notesList);

        // Then progress indicator is hidden and notes are shown in UI
        InOrder inOrder = Mockito.inOrder(notesView);
        inOrder.verify(notesView).setProgressIndicator(true);
        inOrder.verify(notesView).setProgressIndicator(false);
        verify(notesView).showNotes(notesList);
    }

    @Test
    public void clickOnFab_ShowsAddsNoteUi() {
        // When adding a new note
        // this executes the method addNewNote
        notesPresenter.addNewNote();

        // Then add note UI is shown
        // Verify checks if the method showAddNote was called on last step
        verify(notesView, times(1)).showAddNote();
    }

    @Test
    public void clickOnNote_ShowsDetailUi() {
        // Given a stubbed note
        Note requestedNote = new Note("Details Requested", "For this note");

        // When open note details is requested
        notesPresenter.openNoteDetails(requestedNote);

        // Then note detail UI is shown
        verify(notesView).showNoteDetailUi(any(String.class));
    }
}
